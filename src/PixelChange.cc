...
Napi::Value CompareGrayPixels(const Napi::CallbackInfo& info) {
    ...
    for (uint_fast32_t y = 0, i = 0; y < height; y++) {
        for (uint_fast32_t x = 0; x < width; x++, i++) {
            if (abs(data0[i] - data1[i]) >= diff) count++;
        }
    }
    Napi::Number num = Napi::Number::New(env, 100 * count / wxh);
    return num;
}
...
Napi::Value CompareRgbPixels(const Napi::CallbackInfo& info) {
    ...
    for (uint_fast32_t y = 0, i = 0; y < height; y++) {
        for (uint_fast32_t x = 0; x < width; x++, i+=3) {
            if (abs(((data0[i] + data0[i+1] + data0[i+2])/3) - ((data1[i] + data1[i+1] + data1[i+2])/3)) >= diff) count++;
        }
    }
    Napi::Number num = Napi::Number::New(env, 100 * count / wxh);
    return num;
...
Napi::Value CompareRgbaPixels(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (info.Length() != 5 || !info[0].IsNumber() || !info[1].IsNumber() || !info[2].IsNumber() || !info[3].IsBuffer() || !info[4].IsBuffer() ) {
        Napi::TypeError::New(env, "Must be 5 args passed as width, height, diff, buffer0, buffer1").ThrowAsJavaScriptException();
        return env.Null();
