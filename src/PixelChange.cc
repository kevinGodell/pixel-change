...
Napi::Array CompareGrayRegions(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() != 5 || !info[0].IsNumber() || !info[1].IsNumber() || !info[2].IsArray() || !info[3].IsBuffer() || !info[4].IsBuffer() ) {
        Napi::TypeError::New(env, "Must be 5 args passed as width(unsigned int), height(unsigned int), regions(Array), buffer0(Buffer), buffer1(Buffer").ThrowAsJavaScriptException();
    ...
    for (uint32_t i = 0; i < regionsLen; i++) {
        if (
            ...
            !regionsArr.Get(i).As<Napi::Object>().Has("difference") ||
            ...
            !regionsArr.Get(i).As<Napi::Object>().Get("difference").IsNumber() ||
            !regionsArr.Get(i).As<Napi::Object>().Get("count").IsNumber() ||
            !regionsArr.Get(i).As<Napi::Object>().Get("bitset").IsBuffer()
        )
        {
            Napi::TypeError::New(env, "Regions must contain 4 properties: name, difference, count, and bitset.").ThrowAsJavaScriptException();
            return Napi::Array::New(env);
        }
        const std::string name = regionsArr.Get(i).As<Napi::Object>().Get("name").As<Napi::String>();
        const uint8_t difference = regionsArr.Get(i).As<Napi::Object>().Get("difference").As<Napi::Number>().Uint32Value();
        if (difference < 1 || difference > 255) {
            Napi::TypeError::New(env, "Difference value must range from 1 to 255.").ThrowAsJavaScriptException();
            return Napi::Array::New(env);
        }
        minDiff = std::min(minDiff, difference);
        ...
        regionsVec[i] = std::make_tuple(name, difference, count, bitset, 0);
    }
    for (uint32_t y = 0, p = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++, p++) {
            const uint8_t difference = abs(buf0[p] - buf1[p]);
            if (minDiff > difference) continue;
            for (uint32_t i = 0; i < regionsLen; i++) {
                if (!std::get<3>(regionsVec[i])[p] || difference < std::get<1>(regionsVec[i])) continue;
...
Napi::Array CompareRgbRegions(const Napi::CallbackInfo& info) {
    ...
    for (uint32_t y = 0, i = 0, p = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++, i+=3, p++) {
            const uint8_t difference = abs(buf0[i] + buf0[i+1] + buf0[i+2] - buf1[i] - buf1[i+1] - buf1[i+2])/3;
            if (minDiff > difference) continue;
            for (uint32_t i = 0; i < regionsLen; i++) {
                if (!std::get<3>(regionsVec[i])[p] || difference < std::get<1>(regionsVec[i])) continue;
                std::get<4>(regionsVec[i])++;
...
Napi::Array CompareRgbaRegions(const Napi::CallbackInfo& info) {
    ...
    for (uint32_t y = 0, i = 0, p = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++, i+=4, p++) {
            const uint8_t difference = abs(buf0[i] + buf0[i+1] + buf0[i+2] - buf1[i] - buf1[i+1] - buf1[i+2])/3;
            if (minDiff > difference) continue;
            for (uint32_t i = 0; i < regionsLen; i++) {
                if (!std::get<3>(regionsVec[i])[p] || difference < std::get<1>(regionsVec[i])) continue;
