...
console.time('=====> testing rgba pixel changes with regions set');
...
p2p.once('pam', (pam)=> {
    ...
    p2p.on('pam', (pam)=> {
        ...
        buf0 = buf1;
        buf1 = pam.pixels;
        console.time('cpp rgba regions');
        const percent0 = PixelChange.compareRgbaRegions(width, height, regions, buf0, buf1);
        console.timeEnd('cpp rgba regions');
...
ffmpeg.on('exit', (code, signal) => {
    ...
    console.timeEnd('=====> testing rgba pixel changes with regions set');
});
